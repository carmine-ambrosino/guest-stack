export function createUserElement(user) {
    ...
    const buttonDiv = createButtonDiv(user.openstack_id);
  ...
  function createButtonDiv(userId) {
    ...
    updateButton.addEventListener("click", () => {
      ...
      // Fill form with user data (use `userId` to fetch the user details if needed)
    });
  
    const deleteButton = createButton("❌", "text-red-500 hover:text-red-600");
    deleteButton.addEventListener("click", () => openDeleteModal(userId));
